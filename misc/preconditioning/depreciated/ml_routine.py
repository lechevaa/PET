...
from misc.preconditioning.depreciated.figures import plot_loss, quality_plots, plot_solver_report
from misc.preconditioning.depreciated.utils import CustomModelCheckpoint, ThresholdCallback, TQDMProgressBar, SavePredictionsCallback, write_tfrecord_kerasify, parse_function_kerasify, parse_function_kerasify_DeepONet, scalers_to_json

from misc.preconditioning.depreciated.utils import allow_kerasify_import, well_models_ready_to_json
...
def fit_well_model_kerasify(n_cells, n_features, well_name, ml_model_folder, data_folder, finetuning, model):
    ...
    for batch in dataset:
        ...
        X_list.append(features.numpy())  # Convert to NumPy and store in list
        y_list.append(labels.numpy())  # Convert to NumPy and store in list

    # Convert lists to NumPy arrays
    X = np.concatenate(X_list, axis=0)  # Combine batches into one large array
    y = np.concatenate(y_list, axis=0)  # Combine batches into one large array

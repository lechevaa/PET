...
from misc.preconditioning.figures import plot_loss, quality_plots, swim_quality_plots
from misc.preconditioning.swim import create_swim, swim_to_keras, test_swim_to_keras_pred
...
def scalers_partial_fit(x, y, dt, resvs, scalers):
    # Assuming X = (Po, Sw, Sg, RS, Kx, Ky, Kz), Y = (Po, Sw, Sg, RS)
    # Sw, Sg do need scaling
    ((scaler_x_p, scaler_x_sw, scaler_x_sg, scaler_x_RS, scaler_Kx), (scaler_y_p, scaler_y_sw, scaler_y_sg, scaler_y_RS), scaler_dt, scaler_resv) = scalers 
    ...
    scaler_x_RS.partial_fit(np.log1p(x[:, :, 3]).reshape(-1, 1))
    scaler_y_RS.partial_fit(np.log1p(y[:, :, 3]).reshape(-1, 1))
    scaler_Kx.partial_fit(np.log10(1e-7 + x[:, :, 4].reshape(-1, 1)))


    scalers = ((scaler_x_p, scaler_x_sw, scaler_x_sg, scaler_x_RS, scaler_Kx), (scaler_y_p, scaler_y_sw, scaler_y_sg, scaler_y_RS), scaler_dt, scaler_resv)
    return scalers 

def create_scalers_fit(x, y, dt, resvs):
    # Assuming X = (Po, So, Sg, RS, Kx), Y = (Po, So, Sg, RS)
    ...
    scaler_x_RS.fit(np.log1p(x[:, :, 3]).reshape(-1, 1))

    scaler_y_RS = MinMaxScaler()
    scaler_y_RS.fit(np.log1p(y[:, :, 3]).reshape(-1, 1))

    scaler_Kx = MinMaxScaler()
    scaler_Kx.fit(np.log10(1e-7 + x[:, :, 4].reshape(-1, 1)))
...
def scalers_scale(x, y, dts, resvs, scalers):
    ...
    y_RS = scalers_y[3].transform(np.log1p(y[:, :, 3]).reshape(-1, 1)).reshape(y_shape)  
    ...
    x_RS = scalers_x[3].transform(np.log1p(x[:, :, 3]).reshape(-1, 1)).reshape(x_shape)  

    x_Kx = scalers_x[4].transform(np.log10(1e-7 + x[:, :, 4].reshape(-1, 1))).reshape(x_shape)

    dts = scaler_dt.transform(np.log10(1e-7 + dts).reshape(-1, 1))
    resvs = scaler_resv.transform(np.log1p(np.abs(resvs)).reshape(-1, 1))

    return np.concatenate([x_p, x_sw, x_sg, x_RS, x_Kx], axis=2), np.concatenate([y_p, y_sw, y_sg, y_RS], axis=2), dts, resvs
...
def fit_well_model_kerasify(X_train, Y_train, n_cells, n_features, well_name, ml_model_folder, finetuning):
    ...
    if not finetuning:
        ...
        epochs = 500
        print("Previous model initial relative l2 error: ", relative_root_mean_squared_error(Y_train, model.predict(X_train)))
...
def fit_swim_well_model_kerasify(X_train, Y_train, n_cells, n_features, well_name, ml_model_folder, finetuning):
    ...
    if finetuning:
        ...
        print("Kerswim Previous model initial relative l2 error: ", relative_root_mean_squared_error(Y_train, previous_model.predict(X_train)))

    swim_model = create_swim()
    swim_model.fit(X_train, Y_train)
    kerswim_model = swim_to_keras(swim_model, n_cells=n_cells, n_features=n_features)
    print("Initial Kerswim Relative L2 error:", relative_root_mean_squared_error(Y_train, kerswim_model.predict(X_train)))

    kerswim_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-10),
                      loss=relative_root_mean_squared_error
                      )
    
    swim_quality_plots(kerswim=swim_model, X_train=X_train, Y_train=Y_train, well_name=well_name, figname='quality_plots',ml_model_folder=ml_model_folder)
    swim_quality_plots(kerswim=kerswim_model, X_train=X_train, Y_train=Y_train, well_name=well_name, figname='quality_plots_kerswim',ml_model_folder=ml_model_folder)
    ...
    # , early_stopping, custom_callback, progress_bar, save_pred
    history = kerswim_model.fit(X_train, Y_train, epochs=epochs, verbose=0, batch_size=64,
            callbacks=[checkpoint, save_pred, progress_bar]
            )
    
    # Kerasify the model
    kerswim_model_path = os.path.join(ml_model_folder, f'swim_{well_name}.model')
    # model is not best model cause of checkpoints, need to load the actual best one 
...
def well_ml_routine(well_name, data_folder, ml_model_folder, finetuning):
    ...
    ## SWIM 
    # swim_model = create_swim()
    # start_swim_fit = time.time()
    # swim_model.fit(X_train[:400, :], Y_train[:400, :])
    # end_swim_fit = time.time()
    # print(f'Time to fit SWIM model: {end_swim_fit - start_swim_fit:.6f}s')
    # kerswim = swim_to_keras(swim_model, n_cells=n_cells, n_features=n_features)
    # print("testing kerswim vs swim")
    # test_swim_to_keras_pred(X_train, Y_train, swim_model, kerswim)
    # kerswim_model_path = os.path.join(ml_model_folder, f'swim_{well_name}.model')
    # export_model(kerswim, kerswim_model_path)

    # swim_quality_plots(kerswim=swim_model, X_train=X_train, Y_train=Y_train, well_name=well_name, figname='quality_plots',ml_model_folder=ml_model_folder)

    # (well_name, loss_hist) = fit_swim_well_model_kerasify(X_train, Y_train, n_cells, n_features, well_name, ml_model_folder, finetuning)
